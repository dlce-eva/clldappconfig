...
from clldappconfig.fabtools.files import is_file
from clldappconfig.fabtools.utils import read_lines, run_as_root
...
def strip_no_lsb(t):
    return t.replace('No LSB modules are available.', '').strip()
    ...
    with settings(hide('running', 'stdout')):
        ...
        if kernel == 'Linux':
            ...
            if is_file('/usr/bin/lsb_release'):
                id_ = strip_no_lsb(run('lsb_release --id --short'))
...
def distrib_release():
    ...
    with settings(hide('running', 'stdout')):
        ...
        if kernel == 'Linux':
            return strip_no_lsb(run('lsb_release -r --short'))
...
def distrib_codename():
    ...
    """
    with settings(hide('running', 'stdout')):
        return strip_no_lsb(run('lsb_release --codename --short'))
...
def distrib_desc():
    ...
    with settings(hide('running', 'stdout')):
        if not is_file('/etc/redhat-release'):
            return strip_no_lsb(run('lsb_release --desc --short'))
