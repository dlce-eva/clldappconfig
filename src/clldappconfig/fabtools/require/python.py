...
from clldappconfig.fabtools.python import (
    create_virtualenv,
    install,
    install_pip,
...
from clldappconfig.fabtools.python_setuptools import (
    install_setuptools,
    is_setuptools_installed,
)
from clldappconfig.fabtools.system import UnsupportedFamily, distrib_family, distrib_codename
...
def setuptools(version=MIN_SETUPTOOLS_VERSION, python_cmd='python'):
    ...
    if not is_setuptools_installed(python_cmd=python_cmd):
        ...
        if family == 'debian':
            require_deb_package('python-dev-is-python3' if distrib_codename() == 'noble' else 'python-dev')
        elif family == 'redhat':
